...
**方言适配**：同一实体在 MySQL 上写 `json` 列（`setString`）、Oracle 写 `CLOB`、SQL Server 写 `nvarchar(max)`，无需改代码。方言由 JDBC 驱动类名自动映射（`org.postgresql.Driver`→PostgreSQL 等），无需配置 `dialect`。保留字列名按方言自动转义、字符串/时间回读按 JDBC 标准适配；Oracle 须经 IDENTITY 列（12c+）生成主键，详见 [orm.md 方言说明](orm.md)。
