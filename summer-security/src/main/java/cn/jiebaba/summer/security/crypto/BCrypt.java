...
final class BCrypt {
    ...
    private static final int[] S_ORIG = {
        ...
        0xcc814544, 0xaf5ebd02, 0xbee3d004, 0xde334afd, 0x660f2807, 0x192e4bb3,
    ...
    static {
        for (int i = 0; i < INDEX_64.length; i++) INDEX_64[i] = -1;
    ...
    private static void init_key(int[] P, int[] S) {
    ...
    private static void encipher(int[] P, int[] S, int[] lr, int off) {
        int c = lr[off];
        int l = lr[off + 1];
        for (int i = 0; i < BLOWFISH_NUM_ROUNDS; ) {
            c ^= P[i++];
            l ^= F(S, c);
            c ^= P[i++];
            l ^= F(S, c);
        }
        lr[off] = l ^ P[BLOWFISH_NUM_ROUNDS];
        lr[off + 1] = c ^ P[BLOWFISH_NUM_ROUNDS + 1];
    }

    private static void expandkey(int[] P, int[] S, byte[] data, int[] off) {
        int[] lr = {0, 0};
        for (int i = 0; i < P.length; i++) {
            P[i] ^= streamtoword(data, off);
        }
    ...
    private static void expandstate(int[] P, int[] S, byte[] data, byte[] key,
        ...
        int[] lr = {0, 0};
        for (int i = 0; i < P.length; i++) {
            P[i] ^= streamtoword(data, dataoff);
        }
        for (int i = 0; i < P.length; i += 2) {
            lr[0] ^= streamtoword(key, keyoff);
            lr[1] ^= streamtoword(key, keyoff);
            encipher(P, S, lr, 0);
            P[i] = lr[0];
            P[i + 1] = lr[1];
        }
    ...
    private static void eks_blowfish_setup(int[] P, int[] S, int cost, byte[] salt, byte[] key) {
        ...
        int[] keyoff = {0};
        init_key(P, S);
        expandstate(P, S, salt, key, dataoff, keyoff);
        int rounds = 1 << cost;
        for (int i = 0; i < rounds; i++) {
            expandkey(P, S, key, keyoff);
            expandkey(P, S, salt, dataoff);
        }
    }
    ...
    static String hashpw(String password, String salt) {
        ...
        if (salt.length() < 29) throw new IllegalArgumentException("Invalid salt: too short");

        char minor;
        int off;
        if (salt.charAt(0) != '$' || salt.charAt(1) != '2') {
            throw new IllegalArgumentException("Invalid salt: must start with $2");
        }
        if (salt.charAt(2) == '$') {
            minor = 0;
            off = 3;
            ...
            if (minor != 'a' && minor != 'b' && minor != 'y' || salt.charAt(3) != '$') {
                throw new IllegalArgumentException("Invalid salt: unsupported version $2" + salt.charAt(2));
            }
            off = 4;
        }

        if (salt.charAt(off + 1) > '9' || salt.charAt(off + 1) < '0'
                || salt.charAt(off) > '9' || salt.charAt(off) < '0') {
            throw new IllegalArgumentException("Invalid salt: bad cost");
        }
        int cost = (salt.charAt(off) - '0') * 10 + (salt.charAt(off + 1) - '0');
        if (cost < 4 || cost > 31) {
            throw new IllegalArgumentException("Invalid salt: cost must be 4..31, was " + cost);
        }
        if (salt.charAt(off + 2) != '$') {
            throw new IllegalArgumentException("Invalid salt: missing $ after cost");
        }

        byte[] saltBytes = decode_base64(salt.substring(off + 3), 16);
        if (saltBytes.length < 16) {
            byte[] padded = new byte[16];
            System.arraycopy(saltBytes, 0, padded, 0, saltBytes.length);
            saltBytes = padded;
        }
        ...
        int[] S = new int[S_ORIG.length];
        eks_blowfish_setup(P, S, cost, saltBytes, passwordBytes);

        int[] cdata = MAGIC_CDATA.clone();
        for (int i = 0; i < 64; i++) {
            for (int j = 0; j < cdata.length; j += 2) {
                encipher(P, S, cdata, j);
            }
        }

        byte[] hashBytes = new byte[BCRYPT_BLOCKS * 4];
        for (int i = 0; i < BCRYPT_BLOCKS; i++) {
            hashBytes[4 * i] = (byte) ((cdata[i] >>> 24) & 0xff);
            hashBytes[4 * i + 1] = (byte) ((cdata[i] >>> 16) & 0xff);
            hashBytes[4 * i + 2] = (byte) ((cdata[i] >>> 8) & 0xff);
            hashBytes[4 * i + 3] = (byte) (cdata[i] & 0xff);
        }

        StringBuilder rs = new StringBuilder(60);
        rs.append("$2");
        rs.append(minor == 0 ? 'a' : minor);
        rs.append('$');
        if (cost < 10) rs.append('0');
        rs.append(cost);
        rs.append('$');
        rs.append(encode_base64(saltBytes, 16));
        rs.append(encode_base64(hashBytes, BCRYPT_BLOCKS * 4 - 1));
        return rs.toString();
    }
    ...
    static String gensalt(int cost, SecureRandom random) {
        ...
        StringBuilder rs = new StringBuilder(29);
        rs.append("$2a$");
        if (cost < 10) rs.append('0');
        rs.append(cost);
        rs.append('$');
        rs.append(encode_base64(salt, 16));
        return rs.toString();
    }
    ...
    static boolean checkpw(String plaintext, String hashed) {
        ...
        String computed;
        try {
            computed = hashpw(plaintext, hashed);
    ...
    private static int char64(char c) {
        if (c < 0 || c >= INDEX_64.length) return -1;
        return INDEX_64[c];
    }

    private static boolean equalsNoEarlyExit(String a, String b) {
