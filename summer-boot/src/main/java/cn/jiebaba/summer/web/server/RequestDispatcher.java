...
public final class RequestDispatcher {
     ...
     * 其中 HandlerException 与 IllegalArgumentException（业务参数校验）映射为 400。
     */
    private void handleException(Throwable t, WebRequest request, WebResponse response) {
        ...
        int status = (t instanceof HandlerException || t instanceof IllegalArgumentException)
                ? HttpStatus.BAD_REQUEST.code() : HttpStatus.INTERNAL_SERVER_ERROR.code();
        response.status(status);
        response.contentType(MediaType.APPLICATION_JSON_UTF8);
