...
public class AiCallLogTest {
    ...
    public void callLogsTokenUsageAndLatency() {
        ...
        Assertions.assertEquals(1, logger.records.size(), "应记录一次调用");
        AiCallLog rec = logger.records.get(0);
        Assertions.assertEquals("deepseek-chat", rec.model());
        Assertions.assertEquals(Long.valueOf(10L), rec.promptTokens());
        Assertions.assertEquals(Long.valueOf(20L), rec.completionTokens());
        Assertions.assertEquals(Long.valueOf(30L), rec.totalTokens());
        Assertions.assertTrue(rec.success(), "成功调用应记录 success=true");
        Assertions.assertTrue(rec.latencyMillis() >= 0, "耗时非负");
        Assertions.assertEquals("hi", rec.querySummary());
    }
    ...
    public void callLogsFailureOnException() {
        ...
        Assertions.assertEquals(1, logger.records.size(), "失败也应记录一次");
        AiCallLog rec = logger.records.get(0);
        Assertions.assertFalse(rec.success(), "失败调用应记录 success=false");
        Assertions.assertNotNull(rec.errorMessage(), "应记录错误信息");
        Assertions.assertNull(rec.model(), "无元数据时 model 为 null");
    }
    ...
    public void streamLogsUsageOnClose() {
        ...
        try (Stream<ChatResponse> s = model.stream(prompt())) {
            ...
            Assertions.assertEquals(2, frames.size(), "应收到 2 个流式帧");
        }

        Assertions.assertEquals(1, logger.records.size(), "关闭 Stream 后应记录一次");
        AiCallLog rec = logger.records.get(0);
        Assertions.assertEquals("deepseek-chat", rec.model());
        Assertions.assertEquals(Long.valueOf(20L), rec.totalTokens());
        Assertions.assertTrue(rec.success());
        Assertions.assertEquals("hi", rec.querySummary());
    }
    ...
    public void loggerExceptionSwallowedAndCallSucceeds() {
        ...
        Assertions.assertEquals("ok", resp.content(), "logger 异常不应影响主调用结果");
    }
